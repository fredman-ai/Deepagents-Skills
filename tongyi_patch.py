        ...
        if "tool_calls" in _dict:
            ...
            for index, value in enumerate(_dict["tool_calls"]):
                if is_chunk:
                    ...
                    try:
                        args = value.get("function", {}).get("arguments")
...
class ChatTongyi(BaseChatModel):
    ...
    def _chat_generation_from_qwen_resp(
        ...
        # --- PATCH START: Fix DeepSeek/DashScope tool_calls string arguments ---
                ...
                if "function" in tc and "arguments" in tc["function"]:
                    ...
                    if isinstance(args, str):
                        try:
                            ...
                            # 如果解析失败，保留原样（可能是非 JSON 格式或已经部分解析）
                            pass
        # --- PATCH END ---

        message = convert_dict_to_message(msg_data, is_chunk=is_chunk)
        if is_last_chunk:
            return dict(
                message=message,
